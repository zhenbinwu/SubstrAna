void compareEfficiencies(string filename1, string filename2, string variable, int nbins, float min, float max, string DenomCut = "", string NumCut = ""){
	...
	jetTree1->Draw(Form("%s>>var1NumH",variable.c_str()), NumCut.c_str(), "goff");
	jetTree2->Draw(Form("%s>>var2NumH",variable.c_str()), NumCut.c_str(), "goff");
	...
	TH1F *ratio1H = var1NumH->Clone("ratio1H");
	ratio1H->Divide(var1NumH, var1DenH, 1, 1, "B");
	TH1F *ratio2H = var2NumH->Clone("ratio2H");
